module DSNN_KNN
    ...
    type SNNS
        snnS :: Array{SparseVector{Int64,Int64},1};
        num_instances :: Int64;
        H :: Dict{Int64, Dict{Int64, Float64}};# used for cache
        KNN :: Int64;
        SNNS() = new();
    end

    function build_snn(ix::ApIndexJoin, KNN::Int64)
        snnobj = SNNS();
        #snnS = spzeros(ix.num_instances, ix.num_instances);
        snnobj.snnS = SparseVector{Int64, Int64}[];
        snnobj.num_instances = ix.num_instances;
        snnobj.H = Dict{Int64, Dict{Int64, Float64}}();
        snnobj.KNN = KNN;
        
        # recall that neighborhoods are sorted by similarity (DESC)
        for q_i in collect(1:ix.num_instances)
            #nbrs_i = spzeros(ix.num_instances);
            #nbrs_i[ map(x->x[2], ix.nbrs[q_i][1:l_i]) ] = 1; # mark KNNeighbors
            maxnn_len = min(snnobj.KNN, length(ix.nbrs[q_i]))
            push!(snnobj.snnS, spzeros(Int64, ix.num_instances));
            snnobj.snnS[q_i][ map(x->x[2], ix.nbrs[q_i][1:maxnn_len]) ] = 1;        
        end
        return snnobj;
    end

    function get_snnsim(i::Int64, j::Int64, snnobj::SNNS)
        if i == j
            return 1.0
        end
        a = min(i,j);
        ...
        if !haskey(snnobj.H, a)
            snnobj.H[a] = Dict{Int64, Float64}();
            nn_in_common = dot(snnobj.snnS[a],snnobj.snnS[b]);
            snnobj.H[a][b] = nn_in_common;
        elseif !haskey(snnobj.H[a], b)
            nn_in_common = dot(snnobj.snnS[a], snnobj.snnS[b]);
            snnobj.H[a][b] = nn_in_common;    
        end    
        
        return snnobj.H[a][b]/snnobj.KNN
    end
